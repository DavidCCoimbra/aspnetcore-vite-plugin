...
const main = () => {
    const manifestPaths = argument(`manifest`) ? [argument(`manifest`)] : (option(`ssr`)
        ? [`./dist/ssr/ssr-manifest.json`, `./dist/ssr/manifest.json`]
        : [`./wwwroot/build/.vite/manifest.json`, `./wwwroot/build/manifest.json`])
    ...
    const manifestDir = dirname(foundManifestPath)
    const buildDir = manifestDir.endsWith('.vite') ? dirname(manifestDir) : manifestDir
    const assetsPath = argument('assets') ?? buildDir+'/assets'
